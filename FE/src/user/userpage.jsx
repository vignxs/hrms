...
const AttendanceCard = () => {
  ...
  const calculateCurrentHours = (punchIn, punchOut) => {
    ...
    if (isNaN(start.getTime()) || (punchOut && isNaN(end.getTime()))) return '00:00:00';
  ...
  useEffect(() => {
    const fetchPunchRecords = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:8000/api/attendance/punch-records/${employeeId}/`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.today) {
            ...
            setPunchInTime(data.today.punch_in_time ? new Date(data.today.punch_in_time) : null);
            setPunchOutTime(data.today.punch_out_time ? new Date(data.today.punch_out_time) : null);
            setTotalHours(data.today.hours_worked ? (data.today.hours_worked) : '00:00:00');
            setAttendanceData(prev => [{
              ...prev[0],
              status: data.today.status,
              reason: data.today.reason,
              punchInTime: data.today.punch_in_time ? new Date(data.today.punch_in_time) : null,
              punchOutTime: data.today.punch_out_time ? new Date(data.today.punch_out_time) : null,
            }, ...prev]);
          }
        }
      } catch (error) {
  ...
  const punchIn = async (reason = '') => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Failed to punch in' }));
  ...
  const punchOut = async (reason = '') => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Failed to punch out' }));
      ...
      <Box display="flex" justifyContent="space-between" alignItems="stretch" mt={2} gap={2}>
        <StyledPaper>
          <Box mb={2}>
            ...
            <Typography variant="h6" fontWeight="bold" sx={{ color: theme.palette.text.primary }}>{formatTime(currentTime)}</Typography>
          </Box>
          <Box position="relative" width={144} height={144} mx="auto">
            ...
            <Box position="absolute" top={0} left={0} right={0} bottom={0} display="flex" alignItems="center" justifyContent="center" flexDirection="column">
              <Typography variant="body2" color="textSecondary" sx={{ color: theme.palette.grey[600] }}>{isPunchedIn ? 'Today' : 'Total'} Hours</Typography>
              <Typography variant="h6" fontWeight="bold" sx={{ color: theme.palette.text.primary }}>{hoursToDisplay}</Typography>
            </Box>
          </Box>
          <Box my={2} display="flex" justifyContent="center" alignItems="center" gap={1}>
            {isPunchedIn && punchInTime ? (
              <>
                ...
                <Typography variant="body2" color="textSecondary" sx={{ color: theme.palette.grey[600] }}>
          ...
          >
            {isPunchOutDisabled ? 'Punched Out Today' : (isPunchedIn ? 'Punch Out' : 'Punch In')}
          </Button>
        </StyledPaper>
        ...
        <Button
          ...
          onClick={async () => {
            try {
              ...
              const response = await fetch(`http://localhost:8000/api/attendance/admin-replies/${employeeId}/`, {
                headers: {
                  'Content-Type': 'application/json',
                  ...(token ? { Authorization: `Bearer ${token}` } : {}),
                },
              ...
              if (response.ok) {
                ...
                ? data[data.length - 1].admin_reply || JSON.stringify(data[data.length - 1])
                : 'No reply found.';
                  ...
                  <TableRow key={row.id} sx={{
                    ...
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        sx={{
                          backgroundColor: row.statusDisplay === 'Present' ? 
                            theme.palette.success.light : 
                            theme.palette.error.light,
                          color: row.statusDisplay === 'Present' ? 
